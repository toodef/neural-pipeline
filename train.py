...
img_processor = ImageProcessor(len(classes), len(train_pathes), [image_size, image_size, 3])
...
def on_epoch():
    ...
    with ImageConveyor(PathLoader(), validation_pathes, 32) as conveyor:
        loss_values = []
        valid_accuracies = []
    ...
    print("Epoch: {}, Training accuracy: {}, Validation accuracy {}, validation loss: {}".format(epoch, accuracy,
                                                                                                 valid_acc, loss))


img_processor.set_on_epoch(on_epoch)

with ImageConveyor(PathLoader(), train_pathes, 32) as conveyor:
    for images in conveyor:
        for img in images:
            img['object'] = cv2.resize(img['object'], (image_size, image_size), 0, 0, cv2.INTER_LINEAR)
